...
import pickle
import mysql.connector as mysqldbconn
...
class Command(BaseCommand):
    ...
    def load_profiles(self, options):
        ...
        db = mysqldbconn.connect(user=username, passwd=password, host=host, db=database)
        cursor = db.cursor()

        # TODO: kolla varför koden hänvisar till en tabell som inte existerar i testdatabasen "survey_surveyuser_user". Finns den i prod?
        surveyusers = {}
        # Av okänd anledning finns här hänvisning till en tabell som inte existerar. oklart varför man har 3st """
        # cursor.execute("""
        ...
        cursor.execute("""
        SELECT SU.id, SU.global_id, SU.deleted, SU.last_participation_id, SU.last_participation_date, SU.name
