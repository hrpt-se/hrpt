...
# TODO: sluta använda osäkra md5, gå över till sha384
# TODO: skaparen av hrpt verkar genomgående ha kallat lokala variabler samma som globala eller importerade variabler, vilket är bäddat för problem, det behöver rättas
...
def _create_token_2022(user):
    ...
    from UUIDv4. Then both are hashed using MD5 digest algorithm.
    """
    new_id = '%d-%s' % (user.id, str(uuid.uuid4()))
    new_hash = hashlib.sha384(new_id.encode('utf-8'))
    new_hash.digest()
